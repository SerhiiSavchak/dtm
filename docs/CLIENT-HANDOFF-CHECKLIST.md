...
| Telegram (заявки) | Сервер: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`. Успіх заявки — будь-який **налаштований** канал | Бот і чат клієнта | Новий бот у BotFather **або** передати існуючий; оновити env; перевірити одну заявку |
| Email (Resend) | **Опційно.** Якщо налаштований і Telegram впав — заявка все одно `200` | Лише якщо клієнт хоче пошту | `RESEND_API_KEY`, `LEADS_TO_EMAIL`, `LEADS_FROM_EMAIL` |
