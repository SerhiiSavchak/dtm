...
import { portfolioCardSize } from "@/lib/portfolio-layout";
import { portfolioCardLabels } from "@/lib/portfolio/labels";
...
function slideAspect(span: Project["span"]) {
...
function slideSizes(span: Project["span"]) {
  return span === "large"
...
export function Projects({ records }: { records: PortfolioRecord[] }) {
  ...
  const cardLabels = useMemo(
    () => records.map((record) => portfolioCardLabels(record, locale, t)),
    [records, locale, t]
  );
          ...
          <div className="project-track">
            {projects.map((project, i) => (
              <ProjectSlide
                ...
                labels={cardLabels[i]}
                lead={i === 0}
                layoutSpan={portfolioCardSize(project.span)}
...
function ProjectSlide({
  ...
  project: Project;
  labels: { title: string; meta: string | null; area: string | null };
  lead: boolean;
      ...
      >
        <div className="project-media">
          <div className="project-media-crop">
            <div
              className={`project-media-frame relative w-full ${slideAspect(layoutSpan)}`}
            >
              {lead ? (
        ...
        <div className="project-slide-caption">
          <div className="project-slide-title min-w-0">
            <p className="type-title text-foreground transition-colors duration-300 group-hover/card:text-accent">
              <span className="inline-flex min-w-0 items-baseline gap-3">
                <span className="min-w-0">{labels.title}</span>
                <span className="project-arrow project-arrow-caption shrink-0" aria-hidden>
                  →
                </span>
              </span>
            </p>
            {labels.meta ? (
              <p className="label mt-1.5 text-muted">{labels.meta}</p>
            ) : null}
          </div>
          {labels.area ? (
            <span className="project-slide-area font-mono type-small tracking-wide text-muted">
              {labels.area}
            </span>
          ) : null}
        </div>
